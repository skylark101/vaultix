...
function StatCard({ label, value, sub }) {
  return (
    <div className="bg-vault-surface border border-vault-border rounded-xl p-4 sm:p-5">
      ...
      <p className="text-xl sm:text-2xl font-semibold text-vault-text break-all">{value}</p>
      {sub && <p className="text-xs sm:text-sm text-vault-subtle mt-1">{sub}</p>}
    </div>
  )
}
...
export default function Dashboard() {
  ...
  if (loading) {
    return (
      <div className="p-6 sm:p-8 text-vault-subtle text-sm">Loading...</div>
    )
  }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 fade-in">
      ...
      {stats?.byType && Object.keys(stats.byType).length > 0 && (
        <div className="bg-vault-surface border border-vault-border rounded-xl p-4 sm:p-5 mb-4 sm:mb-6">
                ...
                return (
                  <div key={type}>
                    <div className="flex justify-between text-sm mb-1.5 gap-2">
                      <span className="text-vault-text truncate">{type}</span>
                      <span className="text-vault-subtle font-mono text-xs sm:text-sm whitespace-nowrap flex-shrink-0">
                        {fmt(amount)}{' '}
                        <span className="text-vault-muted">({pct.toFixed(1)}%)</span>
                      </span>
                    </div>
                    <div className="h-1.5 bg-vault-muted rounded-full overflow-hidden">
                      <div
                        className="h-full bg-vault-accent rounded-full transition-all duration-500"
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
        ...
        {stats?.recentAssets?.length === 0 ? (
          ...
          <div className="divide-y divide-vault-border">
            {stats?.recentAssets?.map(a => (
              <div key={a.id} className="flex items-center justify-between py-3 gap-3 first:pt-0 last:pb-0">
                <div className="min-w-0">
                  <p className="text-sm text-vault-text font-medium truncate">{a.name}</p>
                  <span className="text-xs text-vault-subtle mt-0.5 inline-block">{a.type}</span>
                </div>
                <span className="text-sm font-mono text-vault-text flex-shrink-0">{fmt(a.amountInvested)}</span>
              </div>
            ))}
          </div>
        )}
